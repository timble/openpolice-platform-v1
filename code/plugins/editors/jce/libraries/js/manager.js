...
var Manager = Plugin.extend({
	...
	*/
	...
	getFolderDetails : function(){
		var folder 	= this._selectedItems[this._activeItem];
		var title 	= string.basename(folder.title);
		
		$(this.options.dialog.info).empty().addClass('loader');

		var path = string.path(this._dir, this._encode(title));
		...
		this.xhr('getFolderDetails', path, function(o){
			...
			if($(folder).hasClass('notwritable')){
				comments.include(
			...
			if (o.preview) {										
                ...
                img.onload = function() {
                    ...
                    if ($E('dd', $('info-preview'))) {
                    	$E('dd', $('info-preview')).removeClass('loader').setHTML('<img src="'+ src +'" width="'+ Math.round(w) +'" height="'+ Math.round(h) +'" title="Preview" />');
                    }
                };

	...
	for(itm in this.configuration) {
		if(cfg[itm] != undefined) {
			if (itm.indexOf('color') > 0) { 
				this.configuration[itm] = cfg[itm].substr(-6);
			} else {
...
jeroenwijering.Player.prototype = {
	onLoadHandler: function(pid,tgt,sdr) {
		...
		var ctr = new jeroenwijering.Controller(tgt.configuration);
		var vie = new jeroenwijering.View(tgt.configuration,ctr);
		var mdl = new jeroenwijering.Model(tgt.configuration,ctr,vie);
		ctr.startMVC(vie,mdl);
	}
}

...
class PhocagalleryModelUser extends JModel
{
	...
	function __construct() {
		...
		global $mainframe;
		// SubCategory
		$limit_subcat		= $mainframe->getUserStateFromRequest( $this->_context_subcat.'.list.limitsubcat', 'limitsubcat', 20, 'int' );
		...
		// Image
		$limit_image		= $mainframe->getUserStateFromRequest( $this->_context_image.'.list.limitimage', 'limitimage', 20, 'int' );
	...
	function getDataSubcat($userId) {
		...
		if (empty($this->_data_subcat)) {
			...
			$tree = array();
			$filter_catid		= $mainframe->getUserStateFromRequest( $this->_context_subcat.'.filter_catid',	'filter_catid',	0,	'int' );
			if ($filter_catid == 0) {
	...
	*/
	...
	function _buildContentWhereSubCat($userId) {
		global $mainframe;
		$filter_state		= $mainframe->getUserStateFromRequest( $this->_context_subcat.'.filter_state','filter_state_subcat','',	'word' );
		$filter_catid		= $mainframe->getUserStateFromRequest( $this->_context_subcat.'.filter_catid','filter_catid_subcat',0,'int' );
		$filter_order		= $mainframe->getUserStateFromRequest( $this->_context_subcat.'.filter_order','filter_order_subcat','a.ordering','cmd' );
		$filter_order_Dir	= $mainframe->getUserStateFromRequest( $this->_context_subcat.'.filter_order_Dir','filter_order_Dir_subcat',	'', 'word' );
	...
	function _buildContentWhereImage($userId) {
		global $mainframe;
		$filter_state		= $mainframe->getUserStateFromRequest( $this->_context_image.'.filter_state','filter_state_image','','word' );
		$filter_catid		= $mainframe->getUserStateFromRequest( $this->_context_image.'.filter_catid','filter_catid_image',0,'int' );
		$filter_order		= $mainframe->getUserStateFromRequest( $this->_context_image.'.filter_order','filter_order_image','a.ordering', 'cmd' );
		$filter_order_Dir	= $mainframe->getUserStateFromRequest( $this->_context_image.'.filter_order_Dir','filter_order_Dir_image','', 'word' );

...
class PhocaGalleryRenderMap
{
	...
	function setMarkerIcon($icon) {
		...
		switch ($icon) {
			...
			case 1:
				$imagePath = JURI::base(true).'/components/com_phocagallery/assets/images/mapicons/yellow/';
	...
	function exportZoom($zoom, $value) {
		$js ='var phocaStartZoom 	= '.$zoom.';'."\n"
			...
			.'   '.$value.'.value = phocaZoom;'."\n"
			.'});'."\n";
		return $js;
	}
	
	
	function exportMarker($id, $latitude, $longitude, $valueLat, $valueLng) {
		...
		$js .= 'function exportPoint'.$id.'(phocaPointTmp3) {'."\n"
				.'   '.$valueLat.'.value = phocaPointTmp3.lat();'."\n"
				.'   '.$valueLng.'.value = phocaPointTmp3.lng();'."\n"
				.'}'."\n";

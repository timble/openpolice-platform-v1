...
class PluginsViewPlugins extends JView
{
	function display( $tpl = null )
	{
		...
		$filter_order		= $mainframe->getUserStateFromRequest( "$option.$type.$task.$client.filter_order",		'filter_order',		'p.type',	'cmd' );
		...
		$filter_type		= $mainframe->getUserStateFromRequest( "$option.$type.$task.$client.filter_type", 		'filter_type',		1,			'cmd' );
		...
		$orderby 	= ' ORDER BY '.$filter_order .' '. $filter_order_Dir .', p.row ASC';

		// get the total number of records
		$query = 'SELECT COUNT(p.id)'
		. ' FROM #__jce_plugins AS p'
		. $where
		;
		$db->setQuery( $query );
		$total = $db->loadResult();

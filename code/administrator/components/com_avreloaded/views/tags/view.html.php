 ...
 * Tags View
 */
class AvReloadedViewTags extends JView {
    /**
    ...
    function &_getViewLists() {
        ...
        $filter_order = $app->getUserStateFromRequest("com_avreloaded.filter_tags_order",
            'filter_order', 't.name', 'cmd');
        $filter_order_Dir = $app->getUserStateFromRequest("com_avreloaded.filter_tags_order_Dir",
            'filter_order_Dir', 'ASC', 'word');
        $limit = $app->getUserStateFromRequest('global.list.limit', 'limit',
            $app->getCfg('list_limit'), 'int');
        $limitstart	= $app->getUserStateFromRequest('com_avreloaded.limitstart_tags', 'limitstart', 0, 'int');
